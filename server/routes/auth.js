...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `SELECT u.id, u.email, u.name, u.role, u.salon_id, s.name as salon_name, s.slug as salon_slug, s.timezone, s.show_on_landing, s.phone as salon_phone, s.email as salon_email, s.address as salon_address, s.description as salon_description
    ...
    res.json({
      ...
      salon: { id: u.salon_id, name: u.salon_name, slug: u.salon_slug, timezone: u.timezone || 'Pacific/Auckland', show_on_landing: u.show_on_landing !== false, phone: u.salon_phone, email: u.salon_email, address: u.salon_address, description: u.salon_description },
    });
  } catch (err) {
