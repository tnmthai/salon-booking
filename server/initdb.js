...
// Gift cards table
async function addGiftCardsTable(pool) {
  try {
    await pool.query(`
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS idx_gift_cards_salon ON gift_cards(salon_id)');
    await pool.query(`
    ...
    `);
    console.log('Gift cards tables created');
  } catch (err) {
    console.log('Gift cards tables skipped:', err.message);
  }
}

module.exports = { initDB, seedAdmin, addTimezoneColumn, addStaffActiveColumn, addServiceNameColumn, addUserActiveColumn, addWebsiteColumn, addGiftCardsTable };
